...
func printSubjAltNames(ext pkix.Extension, dnsNames []string, emailAddresses []string, ipAddresses []net.IP, uri []*url.URL, buf *bytes.Buffer) error {
	...
	if len(uri) > 0 {
		buf.WriteString(fmt.Sprintf("%16sURI:%s", "", uri[0].String())) // XXX verify string format
		for i := 1; i < len(uri); i++ {
			buf.WriteString(fmt.Sprintf(", URI:%s", uri[i].String()))
		}
		buf.WriteString("\n")
	}
	return nil
